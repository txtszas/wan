...
function ujian_end() {
	global $ujian_js,$ujian_config;
	if(get_option('ujian_view_mode') == 'all' ){
...
function ujian($content) {
	global $ujian_view_mode,$style_arr,$ujian_js,$ujian_config,$ujian_page_view;

	if((is_single() || is_page() || is_feed()) && $ujian_view_mode != 'all' ){
		if(is_page() && $ujian_page_view != 'yes'){
			return $content;
		}
		if(in_array('slide',$style_arr) && !in_array('widge',$style_arr)){
